...
void idPlayerHand::PrevWeapon()
{
	...
	// Koz dont change weapon if in gui
	if( commonVr->handInGui || !owner->weaponEnabled || owner->spectating || owner->hiddenWeapon || gameLocal.inCinematic || Flicksync_InCutscene || 
		( gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) && owner->bonusChar == BONUS_CHAR_NONE ) || owner->health < 0 )
	{
		return;
	}
...
void idPlayer::UpdateFlashlight()
{
	...
	if( ( gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) && bonusChar == BONUS_CHAR_NONE ) || gameLocal.inCinematic || spectating || fl.hidden )
	{
		worldModel->Hide();
	}
	else
