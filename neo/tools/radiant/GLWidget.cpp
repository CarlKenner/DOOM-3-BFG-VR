...
void idGLDrawableMaterial::draw(int x, int y, int w, int h) {
	...
	if (mat) {
		...
		if (worldDirty) {
			...
			spawnArgs.Set("origin", "0 128 128"); // foresthale 2014-05-16: use 0 128 128 to put light at top left, was 0 0 0 before
			idStr str;
			...
			if (mat->GetNumStages() > 0)
			{
				...
				for ( int i = 0;i < mat->GetNumStages();i++ )
				{
					...
					idImage *checkImg = stage->texture.image;
					if (checkImg && stage->lighting == SL_DIFFUSE)
					{
						img = checkImg;
						break;
					}
		...
		// render it
#if 1 //FIX_RADIANT
		// foresthale 2014-05-10: fixed radiant previews, this isn't super efficient with command buffers but it is very safe
