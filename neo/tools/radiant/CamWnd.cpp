	...
	if ( v && *v ) {
		...
		if ( !stricmp( name, v ) ) {
			...
			if (md5) {
				...
				refent.joints = ( idJointMat * )Mem_Alloc16( refent.numJoints * sizeof( *refent.joints ), TAG_RADIANT );
...
void CCamWnd::Cam_Render() {
	...
	// foresthale 2014-05-10: we don't need to make the context current twice, the code path reaching this function always does it for us
	//if (!wglMakeCurrent(dc.m_hDC, win32.hGLRC)) {
	//	common->Printf("ERROR: wglMakeCurrent failed..\n ");
	//	common->Printf("Please restart " EDITOR_WINDOWTEXT " if the camera view is not working\n");
	//	return;
